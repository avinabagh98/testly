...
def run_comparison(csv_file, pdf_file):
    # 1. Get Data
    df_excel = extract_from_excel(csv_file)
    df_pdf = extract_from_pdf(pdf_file)
    
    # 2. Match on Code
    comparison = pd.merge(df_excel, df_pdf, on='Code', how='outer')
    
    # 3. Exact Matching Logic
    # We use a tiny epsilon for float comparison to avoid 0.00000001 errors, 
    # but logically checking for == 
    ...
    if mismatches.empty:
        ...
        print(mismatches[['Code', 'Excel_Current', 'PDF_Current', 'Excel_Previous', 'PDF_Previous']])

# Filenames
excel_name = 'Ranaghat_BS.xlsx'
pdf_name = 'RanaghaT_old_BS.pdf'

if __name__ == "__main__":
    run_comparison(excel_name, pdf_name)
